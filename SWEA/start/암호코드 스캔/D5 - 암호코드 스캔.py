    ...
    for i in range(N):
        ...
        for j in range(M - 1, -1, -1):  #뒤에서부터 0 아닌값 검색
            if line[j] != '0':  #0이 아닌게 나타나면
                if [i - 1, j] in idx_lst:   #아랫줄 겹치는 암호들 처리
                    idx_lst.append([i, j])
                    i_token = 1
                else:
                    if token == 0 and i_token == 0:
                        idx_lst.append([i, j])  #첫 등장 좌표 기록
                        token = 1
            if j >= 2 and line[j] == '0' and line[j - 1] == '0':    #0이 연달아 나오면 암호 끝 token 0
                token = 0
            ...
            if token == 1:  #token 1이면(암호면)
                ...
                if q:   #큐 차있으면
                    ...
                    for b in range(4):  #암호코드 맨 앞에 0들 필요한 경우 고려
                        q.append('0')
